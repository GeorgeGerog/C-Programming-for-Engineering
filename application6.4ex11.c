...
void bestfit (double x[], double y[], int i_left, int i_right,\
				  double *m, double *b);
void newinput_predict (double xmiddle,double m1,double m2,double b1,double b2);
/*************************************************************************************
**	FUNCTION main
**	CALLS FUNCTIONS TO:
...
int main(void)
{
	...
	newinput_predict (x[i_at_y_max], m1, m2, b1, b2);
	return 0;
}
/*****************************************************************************
**	FUNCTION readinput
...
{
	double x1,y1;
	printf("Please enter vehicle density\n");
	scanf("%lf",&x1);
	...
	printf("the prediction of accidents is %.0lf\n", ceil(y1));
}
